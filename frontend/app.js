...
async function handleExcelUpload(event) {
  ...
  try {
    ...
    showFeedback(elements.excelFeedback, "업로드 완료되었습니다!", false);
...
async function handleTextUpload(event) {
  ...
  if (!file || !(file instanceof File)) {
    showFeedback(elements.textFeedback, "파일을 선택하세요.", true);
    return;
  }

  showFeedback(elements.textFeedback, "업로드 중...", false);
  ...
  try {
    ...
    const payload = await response.json();
    showFeedback(elements.textFeedback, "업로드 완료되었습니다!", false);
...
async function refreshDatasets(selectId) {
  try {
    ...
    if (!response.ok) throw new Error("데이터셋 목록을 불러올 수 없습니다.");
...
async function refreshTexts(selectId) {
  try {
    ...
    if (!response.ok) throw new Error("텍스트 목록을 불러올 수 없습니다.");
...
function renderDatasetList(selectId) {
  ...
  if (!state.datasets.length) {
    container.innerHTML = `<li>업로드된 데이터셋이 없습니다.</li>`;
  ...
  state.datasets.forEach((dataset) => {
    ...
    item.innerHTML = `
      ...
      <small>${formatDate(dataset.uploaded_at)} | ${dataset.row_count}행 / ${dataset.column_count}열</small>
    `;
...
async function displayDataset(datasetId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("데이터셋을 불러올 수 없습니다.");
...
function renderTextList(selectId) {
  ...
  if (!state.texts.length) {
    container.innerHTML = `<li>업로드된 텍스트가 없습니다.</li>`;
...
async function displayText(textId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("텍스트를 불러올 수 없습니다.");

...
async function handleExcelUpload(event) {
  ...
  if (!file || !(file instanceof File)) {
    showFeedback(elements.excelFeedback, "파일을 선택하세요.", true);
    return;
  }

  showFeedback(elements.excelFeedback, "업로드 중...", false);
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: "?쒕쾭 ?ㅻ쪟媛 諛쒖깮?덉뒿?덈떎." }));
      throw new Error(error.error || response.statusText);
    }

    const payload = await response.json();
    showFeedback(elements.excelFeedback, "?낅줈???꾨즺?섏뿀?듬땲??", false);
...
async function handleTextUpload(event) {
  ...
  if (!file || !(file instanceof File)) {
    showFeedback(elements.textFeedback, "?뚯씪???좏깮?섏꽭??", true);
    return;
  }

  showFeedback(elements.textFeedback, "?낅줈??以?..", false);
  ...
  try {
    ...
    const payload = await response.json();
    showFeedback(elements.textFeedback, "?낅줈???꾨즺?섏뿀?듬땲??", false);
...
async function refreshDatasets(selectId) {
  try {
    ...
    if (!response.ok) throw new Error("?곗씠?곗뀑 紐⑸줉??遺덈윭?????놁뒿?덈떎.");
...
async function refreshTexts(selectId) {
  try {
    ...
    if (!response.ok) throw new Error("?띿뒪??紐⑸줉??遺덈윭?????놁뒿?덈떎.");
...
function renderDatasetList(selectId) {
  ...
  if (!state.datasets.length) {
    container.innerHTML = `<li>?낅줈?쒕맂 ?곗씠?곗뀑???놁뒿?덈떎.</li>`;
...
async function displayDataset(datasetId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("?곗씠?곗뀑??遺덈윭?????놁뒿?덈떎.");
...
function renderDatasetTable(payload) {
  ...
  if (!payload?.data?.length) {
    container.innerHTML = `<p>?쒖떆???곗씠?곌? ?놁뒿?덈떎.</p>`;
    return;
  }
...
function renderTextList(selectId) {
  ...
  if (!state.texts.length) {
    container.innerHTML = `<li>?낅줈?쒕맂 ?띿뒪?멸? ?놁뒿?덈떎.</li>`;
...
async function displayText(textId) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("?띿뒪?몃? 遺덈윭?????놁뒿?덈떎.");
